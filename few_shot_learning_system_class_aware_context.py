    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase):
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(zip(x_support_set,
            ...
            for num_step in range(num_steps):
                ...
                # z를 업데이트를 위한 샘플별 손실
                sample_losses = F.cross_entropy(input=support_preds,
                ...
                for class_id_tensor in unique_classes:
                    ...
                    mask = (y_support_set_task == class_id_tensor)

                    # 해당 클래스의 샘플 손실만 평균 (z 업데이트 경사 계산에 사용)
                    class_loss = torch.mean(sample_losses[mask])

                    z_grads = torch.autograd.grad(class_loss, [z],
                                                  create_graph=use_second_order,
                                                  retain_graph=True,
                                                  allow_unused=True)[0]

                    if z_grads is not None:
                        # P_grads의 해당 클래스 행만 추출하여 업데이트
                        # P_grads는 (ncs, D_emb) 모양
                        updated_z[class_id] = current_z[class_id] - self.args.class_embedding_learning_rate * z_grads[class_id]

                z = updated_z  # z 업데이트 완료
