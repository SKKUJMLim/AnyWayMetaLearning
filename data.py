...
class FewShotLearningDatasetParallel(Dataset):
    ...
    def get_set(self, dataset_name, seed, augment_images=False, num_classes_override=None):
        """
        ...
        # --- N 결정: override가 있으면 그것을 우선 사용 ---
        if num_classes_override is not None:
            num_classes_this_episode = int(num_classes_override)
        else:
            if self.any_way_setting:
                # (fallback) 혹시 override가 빠지면 기존 상태변수 사용
                num_classes_this_episode = int(getattr(self, 'num_classes_per_set_current', self.num_classes_per_set))
            else:
                num_classes_this_episode = int(self.num_classes_per_set)

        # 사용 가능한 클래스 수를 초과하지 않도록 안전장치
        all_classes = list(self.dataset_size_dict[dataset_name].keys())
        num_classes_this_episode = min(num_classes_this_episode, len(all_classes))

        # ★ 변경: 에피소드에 사용할 클래스 선택 시 num_classes_this_episode 사용
        selected_classes = rng.choice(all_classes, size=num_classes_this_episode, replace=False)
        rng.shuffle(selected_classes)

        # --- [클래스별 회전 k] ---------------------------------------------------
        # ★ 변경: 크기를 num_classes_this_episode로
        k_list = rng.randint(0, 4, size=num_classes_this_episode)
        k_dict = {selected_class: k_item for (selected_class, k_item) in zip(selected_classes, k_list)}

        # --- [에피소드 라벨 매핑: 0..N-1] --------------------------------------
        # ★ 변경: 라벨 길이를 num_classes_this_episode로
        episode_labels = [i for i in range(num_classes_this_episode)]
        class_to_episode_label = {
            selected_class: episode_label
    ...
    def switch_set(self, set_name, current_iter=None):
        ...
        # 항상 기본값으로 초기화 (train에선 __getitem__에서 per-batch로 override)
        self.num_classes_per_set_current = self.num_classes_per_set
    ...
    def __getitem__(self, idx):
        # 배치 그룹 단위로 N 샘플링
        n_override = self._sample_any_way_N_for_index(self.current_set_name, idx)

        # 배치 시작에서만 간단히 로깅
        if (idx % self.effective_batch_size) == 0 and self.any_way_setting and self.current_set_name in ("train", "val"):
            pass
            # print(f"[Data] {self.current_set_name} batch start @ idx={idx}: ANY-WAY N={n_override}")

        support_set_images, target_set_image, support_set_labels, target_set_label, seed = \
            self.get_set(self.current_set_name,
                         seed=self.seed[self.current_set_name] + idx,
                         augment_images=self.augment_images,
                         num_classes_override=n_override)
